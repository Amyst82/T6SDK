...
{
	class UI_RectangleClickableButton
	{
	private:
		float fadingX = 0.0f;
	...
	public:
		...
		UI_RectangleClickableButton(const char* text, float x, float y, bool drawRelative, T6SDK::AnchorPoint anchorPoint, bool cyclingFading, std::function<void()> Func)
		{
			Text = text;
			X = x;
			Y = y;
			...
			WIDTH = T6SDK::InternalFunctions::R_TextWidth(text, strlen(text), T6SDK::Drawing::normalFont) + 50.0f;
			HEIGHT = T6SDK::Drawing::normalFont->pixelHeight + 30.0f;
		}
		/// <summary>
		/// Clickable button with icon
		/// </summary>
		...
		/// <param name="anchorPoint"></param>
		UI_RectangleClickableButton(const char* text, int gridColumn, int gridRow, T6SDK::AnchorPoint anchorPoint, bool cyclingFading, std::function<void()> Func)
		{
			Text = text;
		...
		void Draw()
		{
			...
			if (DrawRelative)
			{
				float scale = 1.2f / 1080.0f * static_cast<float>(T6SDK::Addresses::ScreenHeight.Value()); //Assuming that initial size is in 1920x1080			
			...
			if (successDraw)
			{
				...
				//Drawing text
				float textX = (float)baseRect.left + (((float)baseRect.right - (float)baseRect.left) / 2.0f);
