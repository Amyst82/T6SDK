...
namespace T6SDK::Drawing
{
	...
	public:
		...
		UI_ListBox(const char* title, float x, float y, float width, float height, bool drawRelative, vector<const char*>* items, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
			X = x;
			Y = y;
		...
		/// <param name="anchorPoint"></param>
		/// <param name="function">Pointer to a function that will be called when the selected item is changed. void func(int)</param>
		UI_ListBox(const char* title, int gridColumn, int gridRow, float width, float height, vector<const char*>* items, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
		...
		UI_ListBox(const char* title, int gridColumn, int gridRow, int gridColumn2, int gridRow2, vector<const char*>* items, int* selectedIndex, T6SDK::AnchorPoint anchorPoint, uintptr_t function)
		{
			Title = title;
			GridColumn = gridColumn;
			GridRow = gridRow;
		...
		void Draw()
		{
			...
			float titleHeight = (float)T6SDK::Drawing::normalFont->pixelHeight * scale;
			float titleX = (float)bgRect.left + (m_width / 2.0f);
			float titleY = (float)bgRect.top - titleHeight;
			T6SDK::Drawing::DrawTextAbsolute(Title, titleX, titleY, 1.0f, T6SDK::Drawing::WHITECOLOR, T6SDK::AnchorPoint::TopCenter, 0x00);
			...
			for (int i = startIndex; i < endIndex; ++i) 
			{
				...
				// Draw the text
				RECT textRect{};
				T6SDK::Drawing::DrawTextAbsolute((*Items)[i], bgRect.left + 5, itemY + 0, 1.0f, !IsEnabled ? T6SDK::Drawing::GRAYCOLOR : Hovered ? T6SDK::Drawing::ORANGECOLOR : T6SDK::Drawing::WHITECOLOR, T6SDK::AnchorPoint::TopLeft, &textRect);
			}
			// Draw the scrollbar
